...
const generateSeatingPlan = async ({
  ...
  try {
    ...
    // Step 3 — Assign seats (with proper break handling)
    roomLoop: for (let room of rooms) {
      ...
      for (let r = 1; r <= rows; r++) {
        for (let c = 1; c <= cols; c++) {
          ...
          if (studentIndex >= students.length) {
            break roomLoop; // ❗ IMPORTANT: Break from all loops using labeled break
          }
    ...
    // Add empty seats for remaining positions
    ...
    logger.info(`Seating plan generation completed: ${seats.length} total seats, ${seats.filter(s => s.status === 'assigned').length} assigned`);
